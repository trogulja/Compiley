...
function calculateDays(db) {
  const sql = db.prepare(
    'UPDATE days SET sum_images = ( SELECT sum(amount) FROM jobs WHERE jobs.days = days.id ), sum_users = ( SELECT count(DISTINCT (metaUsers) ) FROM jobs WHERE jobs.days = days.id AND jobs.metaUsers IS NOT NULL ), sum_images_atomic = ( SELECT sum(amount) FROM ( SELECT count( * ) AS amount FROM jobsAtomic LEFT JOIN jobs ON jobsAtomic.jobs = jobs.id WHERE jobs.days = days.id UNION SELECT count( * ) AS amount FROM helperClaro WHERE helperClaro.days = days.id AND helperClaro.type IN (3, 4, 5, 6) AND helperClaro.metaUsers IS NOT NULL ) ), sum_users_atomic = ( SELECT count( * ) FROM ( SELECT DISTINCT (jobs.metaUsers) FROM jobsAtomic LEFT JOIN jobs ON jobsAtomic.jobs = jobs.id WHERE jobs.days = days.id AND jobs.metaUsers IS NOT NULL UNION SELECT DISTINCT metaUsers FROM helperClaro WHERE helperClaro.days = days.id AND helperClaro.type IN (3, 4, 5, 6) AND helperClaro.metaUsers IS NOT NULL ) ), sum_d_working = ( SELECT sum(duration) FROM jobs WHERE jobs.days = days.id ), sum_d_presence = ( SELECT sum(d_presence) FROM worktime WHERE worktime.days = days.id );'
  );
